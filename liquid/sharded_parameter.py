...
class ShardedParameter(Parameter):
    ...
    def _get_shard(self, tensor: torch.Tensor, shard_id: int) -> torch.Tensor:
        ...
        shard = self.narrow(self.shard_dim, start_index, self.shard_size)
        return shard
    ...
    def _delete_shard(self, tensor: torch.Tensor, shard_id: int) -> torch.Tensor:
        ...
        before_shard = self.narrow(self.shard_dim, 0, start_index)
        after_shard = self.narrow(self.shard_dim, start_index + self.shard_size, self.size(self.shard_dim) - start_index - self.shard_size)
    ...
    def _append_shard(self, shard_id: int, src_data: torch.Tensor, appended_data: torch.Tensor) -> torch.Tensor:
    ...
    def append_shard(self, shard_id: int, shard_data: torch.Tensor) -> None:
        ...
        new_data = self._append_shard(shard_id, self.data, shard_data)
        self.data = new_data
