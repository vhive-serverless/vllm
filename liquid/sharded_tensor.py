...
class ShardedTensor(Tensor):
    ...
    def __init__(self, 
        ...
        assert self.shape[self.shard_dim] % self.num_shards == 0, f"Tensor's {self.shard_dim} dim with length: {self.shape[self.shard_dim]} is not divisible by number of shards: {self.num_shards}, tensor's shape: {self.shape}"
