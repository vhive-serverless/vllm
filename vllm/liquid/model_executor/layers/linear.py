...
class LiquidLinearMethod(LinearMethodBase):
...
class ColumnParallelLinear(LinearBase):
    ...
    def __init__(self,
        ...
        super().__init__(input_size, output_size, skip_bias_add, params_dtype,
                         quant_config)
        self.quant_method: LiquidLinearMethod = LiquidLinearMethod()
        ...
        assert self.quant_method is not None
        self.output_size_per_partition = divide(self.output_size, total_num_shards) * current_num_shards
        self.output_partition_sizes = [self.output_size_per_partition]
        # If QKV or MergedColumn, use output size of each partition.
        if hasattr(self, "output_sizes"):
            self.output_partition_sizes = [
                divide(output_size, total_num_shards) * current_num_shards
                for output_size in self.output_sizes
            ]

        if output_sizes is None:
            output_sizes = [output_size]
        # For column parallel linear, tensor is shareded along first dimension
        shard_dim = 0
...
class QKVParallelLinear(ColumnParallelLinear):
    ...
    def __init__(self,
        ...
        # tp_size = get_tensor_model_parallel_world_size()
        current_num_shards = len(shard_ids)
        self.num_heads = divide(self.total_num_heads, total_num_shards) * current_num_shards
        # if tp_size >= self.total_num_kv_heads:
        #     self.num_kv_heads = 1
        #     self.num_kv_head_replicas = divide(tp_size,
        #                                        self.total_num_kv_heads)
        # else:
        self.num_kv_heads = divide(self.total_num_kv_heads, total_num_shards) * current_num_shards
        self.num_kv_head_replicas = 1

        input_size = self.hidden_size
        output_size = (self.num_heads +
                       2 * self.num_kv_heads) // current_num_shards * self.head_size * total_num_shards
        self.output_sizes = [
            self.num_heads * self.head_size // current_num_shards * total_num_shards,  # q_proj
            self.num_kv_heads * self.head_size // current_num_shards * total_num_shards,  # k_proj
            self.num_kv_heads * self.head_size // current_num_shards * total_num_shards,  # v_proj 
        ]
...
class RowParallelLinear(LinearBase):
    ...
    def __init__(self,
        ...
        current_num_shards: int = len(shard_ids)
        self.quant_method: LiquidLinearMethod = LiquidLinearMethod()
        ...
        self.input_size_per_partition = divide(input_size, total_num_shards) * current_num_shards
        assert self.quant_method is not None
        # row parallel linear is sharded along its second dimension
        shard_dim = 1
        self.quant_method.create_weights(
            layer=self,
        ...
        if bias:
            self.bias = Parameter(
                data=torch.empty(self.output_size, dtype=params_dtype),
            )
            set_weight_attrs(self.bias, {
                "output_dim": 0,
                "weight_loader": self.weight_loader,
