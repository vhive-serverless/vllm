...
@asynccontextmanager
async def build_async_engine_client(shared_dict, gpu_ids, instance_uuid,
        args: Namespace) -> AsyncIterator[EngineClient]:

    # Context manager to handle engine_client lifecycle
    # Ensures everything is shutdown and cleaned up on error/exit
    engine_args = AsyncEngineArgs.from_cli_args(args)
    engine_args.distributed_executor_backend = LiquidExecutor

    async with build_async_engine_client_from_engine_args(
            shared_dict, gpu_ids, instance_uuid, engine_args, args.disable_frontend_multiprocessing ) as engine:
        yield engine
...
async def build_async_engine_client_from_engine_args(
    ...
    # Otherwise, use the multiprocessing AsyncLLMEngine.
        ...
        engine_process = context.Process(target=run_mp_engine,
                                         args=(engine_args,
                                               UsageContext.OPENAI_API_SERVER,
                                               ipc_path, engine_alive, shared_dict, gpu_ids, instance_uuid))
...
async def run_server(shared_dict, gpu_ids, instance_uuid, args, **uvicorn_kwargs) -> None:
    ...
    async with build_async_engine_client(shared_dict, gpu_ids, instance_uuid, args) as engine_client:
        app = build_app(args)
